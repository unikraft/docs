...
Along with the [`run-app-elfloader`](https://github.com/unikraft/run-app-elfloader) repository, we have collected pre-built applications that you can use in binary compatibility mode.
...
These are pre-built applications, eliminating the need for compilation.
...
Recall that applications that require networking support (i.e. those where the `-n` option is passed to the `run.sh` script) need to be run with admin rights;
use `sudo` in front of the the `run_app.sh` commands.
...
As you've seen, running an application in binary compatibility mode requires a filesystem (storing the Linux binary, dynamic libraries and support files) and the command line used to start the application.
...
Only the required files are added to it, similar to a container, making the resulting image as small as possible.
