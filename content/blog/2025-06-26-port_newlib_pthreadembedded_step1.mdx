---
title: "Update Newlib and Pthread-embedded Libraries - 1–4 Progress blog"
description: |
...
However, **Unikraft**, as a unikernel development framework, aims to behave more like a fully functional operating system — especially when running Linux-like applications. This means Unikraft needs to support not only ISO C but also a wide range of **POSIX APIs**, which define system calls and interfaces commonly used in Unix-like operating systems.
...
We therefore chose to port **Newlib 4.5 (released in 2024)** — the latest version at the time of writing — to ensure:
...
1. **Extract only the relevant source files** from Newlib’s source tree, based on our target architecture and desired features.
...
This requires analyzing Newlib’s configuration files and build logic to determine:
...
Manually handling this for 800–1200 source files would be highly error-prone, so part of our effort involves **automating** or simplifying this extraction and build definition process.

## A Smarter Approach: Extracting the Minimal Build Set

As discussed earlier, we can't simply copy all of Newlib's source files into Unikraft and compile them — doing so would not only be inefficient, but also result in duplicate symbol errors, configuration mismatches, and linker conflicts.

Manually identifying and importing the correct subset of files (from a pool of around **800–1200 source files**, depending on configuration) is equally impractical.

One possible (but extreme) idea would be to **rebuild Newlib’s build system from scratch inside Unikraft**, replicating its behavior using `Makefile.uk` syntax. But this would be a massive undertaking — essentially reverse-engineering all of Newlib’s `autotools` logic.

## A Smarter Approach: Extracting the Minimal Build Set

As discussed earlier, directly copying all of Newlib's source files into Unikraft and compiling them is not practical. This would lead to configuration mismatches, duplicate symbol errors, and excessive code inclusion. On the other hand, manually selecting the necessary 800–1200 source files would be extremely time-consuming and error-prone.

One might consider re-implementing Newlib’s entire build logic using Unikraft’s build system (`Makefile.uk`), but that would essentially mean reverse-engineering Newlib’s complex autotools-based setup — a massive undertaking.

## A Smarter Trick: Using Verbose Make Output

Newlib's build system is powered by GNU autotools — specifically `autoconf` and `automake`. These tools generate Makefiles based on the configuration options passed in. This makes Newlib highly flexible, supporting many target platforms and feature sets.

Here’s the insight: when Newlib is built in **verbose mode**, it prints every `gcc` invocation, showing exactly which source files are compiled, with which compiler flags and macro definitions.

### 🛠️ Step 1: Configure Newlib
...
../newlib-4.5.0.20241231/configure --target=x86_64-elf \
    ...
    --disable-newlib-mb
```
#### 🔧 Explanation of `configure` Flags

```bash
--disable-multilib
```
Build only for 64-bit (`x86_64`); no need for multiple ABIs.

```bash
--enable-newlib-reent-thread-local
```
Enables use of thread-local storage for reentrancy. This is supported by Unikraft's thread model.

```bash
--enable-newlib-io-c99-formats
```
Enables C99 format specifiers in `printf` and `scanf` (e.g., `%ll`, `%z`, `%j`). Required for modern C compatibility.

```bash
--enable-newlib-io-long-long
```
Adds support for printing and scanning `long long` values (`%lld`, `%llu`).

```bash
--enable-newlib-elix-level=1
```
Enables EL/IX Level 1 compliance: small footprint with essential POSIX features. Higher levels may conflict with Unikraft and `pthread-embedded`.

```bash
--disable-libssp
```
Disables GCC’s Stack Smashing Protector runtime. Unikraft already provides this as a separate library.

```bash
--disable-newlib-complex-math
```
Disables complex math functions, which depend on newer compiler builtins. Avoids compatibility issues with older compilers.

```bash
--disable-newlib-supplied-syscalls
```
Prevents Newlib from defining its own syscall stubs (`_read`, `_write`, etc.). Unikraft provides its own syscall shim layer.
...
--disable-newlib-mb
```
These features are currently disabled to simplify the integration, but can be added in future updates if needed.


This step creates a tailored build environment for just the features we want, eliminating unnecessary modules.

### 🛠️ Step 2: Build in Verbose Mode
...
- ✅ The list of actual source files compiled
- ✅ Important macro definitions (`-D` flags)
- ✅ Header include paths (`-I`, `-isystem`)
- ✅ Any internal assumptions about system call behavior
...
By reusing this compile-time information, we avoid rewriting Newlib’s build system from scratch and instead create an efficient and reliable integration workflow that aligns with Unikraft’s modular build process.

## Automating the Integration with a Script

To efficiently transport Newlib’s build logic into Unikraft and avoid manual effort, we developed a Python script that **parses the verbose build output** and extracts:
...
This script then uses that data to generate entries for a Unikraft-style `Makefile.uk`, which is used to define how the library should be built inside Unikraft’s ecosystem.

By automating this process, we eliminate the need to manually sift through hundreds of compiler commands or reverse-engineer Newlib’s complex configuration system. The script ensures we only include exactly what’s needed — nothing more, nothing less.
...
2. **Resolve conflicts** between Newlib’s symbols and existing Unikraft components.
3. Ensure Unikraft’s build system can compile and link Newlib as part of a working unikernel image.

With the foundation in place, we can now focus on compatibility, testing, and refinement — ensuring that Newlib works seamlessly alongside or as an alternative to musl within the Unikraft environment.
